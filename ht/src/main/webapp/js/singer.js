...
function load(){
	$.get("singer/s",function(data){
		var Singerinfo=[];
		for(var i = 0; i <data.length; i++){
			Singerinfo.push(data[i].sgnation+data[i].sggender);
		}
		var result = [], hash = {};
		for (var i = 0, elem; (elem = Singerinfo[i]) != null; i++) {
			if (!hash[elem]) {
				result.push(elem);
				hash[elem] = true;
			}
		...
		for(var i = 0; i <result.length; i++){
			if(result[i]=='null男' || result[i]=='null女'){
			}else if(i<10){
...
//查询男歌手
function sggender(sggender){
	$.get("singer/"+sggender,function(data){
		var c="";
		var c1="";
		var c2="";
		var c3="";
