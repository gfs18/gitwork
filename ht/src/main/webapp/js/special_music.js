...
function specialDetail(){
	var spid=location.href.split("?")[1].split("=")[1];
	//alert(spid);
	$.get("special/detail",{"spid":spid},function(data){
		...
		spDetailStr+='<p class="info_3">发行时间：'+data.sppubTime+'</p><p class="info_4">语种：'+data.language+'</p>';
	...
	$.get("special/music",{"spid":spid},function(data){
		...
		for(var i=0; i<data.length;i++){
			musicStr+='<div class="row"><div  class="list_no col-lg-1 col-md-1 col-sm-1 col-xs-1">'+(i+1)+'</div>'
	            ...
	            +'<div  class="list_play col-lg-2 col-md-2 hidden-sm hidden-xs">'+data[i].soname+'</div>'
