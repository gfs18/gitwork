      ...
      <div class="photo-box">
            <div class="mod_focus_show" id="divimgplay">
                <div class="lyric-mask">
                  <div class="lyric-box">
                      <div class="lyric-content" id="show-lrc-content">
                      </div>
                  </div>            
                </div>
                ...
                <ul id="photo-descriotion" class="mod_focus_title" id="ptitleg_imgPlayer">
                </ul>
            </div>
        </div>
...
//        'music/yixiaoqingcheng.lrc',
    ...
    $(function(){
        var oid=location.hash.replace("#","");
        $.post("/getAllCunSong",{oid:oid},function (){

        });
        getAllSong(oid);
    })
    function getAllSong(id){
        var myid=id;
        $.get("/getAllSong",function (data) {
            if(data.error){
                alert("查询文章失败.....");
            }else {
            var str="";
                ...
                $.each(data, function (index,item) {
                    str+='<li class="m_list"  onclick="show('+index+',this)">'+item.oname+' - '+item.sname+'</li>';
                    if(item.oid==myid){
                       jilu=index;
                    }
                    musicSrcs.push(item.osrc);
                    lyricSrcs.push(item.ogeci);
                });
                $("#myul").append(str);
                myshow(jilu);
            }
